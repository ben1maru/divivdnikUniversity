...
public class CRUDUniverse {
    ...
    private void loadUniversities() {
        try {
            String query = "SELECT name FROM universe"; // Запит до бази даних
    ...
    private void addButton(String universityName) {
        ...
        editButton.setOnAction(event -> {
            ...
            loader.setLocation(getClass().getResource("EditUniverse.fxml"));
            try {
                loader.load();
                EditUniverse controller = loader.getController(); // Отримуємо контролер іншої форми
        ...
        deleteButton.setOnAction(event -> deleteUniversity(universityName));
    ...
    private void deleteUniversity(String universityName) {
        // SQL query to delete the university based on its name
        String deleteQuery = "DELETE FROM `universe` WHERE `name` = ?";
        ...
        try (PreparedStatement preparedStatement = db.getDbConnection().prepareStatement(deleteQuery)) {
            ...
            if (affectedRows > 0) {
                ...
                alert.show();
                System.out.println("University '" + universityName + "' deleted successfully.");
            } else {
                System.out.println("Failed to delete university '" + universityName + "'.");
